...
<form action="" method="GET" class="form-inline" role="form">
    ...
    <option value="name-ASC" {{$order == 'name-ASC'?'selected':''}}>Sắp xếp tên từ a - z</option>
    <option value="name-DESC" {{$order == 'name-DESC'?'selected':''}}>Sắp xếp tên từ z - a</option>
    <option value="price-ASC" {{$order == 'price-ASC'?'selected':''}}>Sắp xếp giá từ thấp - cao</option>
    <option value="price-DESC" {{$order == 'price-DESC'?'selected':''}}>Sắp xếp giá từ cao - thấp</option>
    </select>
    </div>
...
<td>{{$pro->status != 0 ? "Hiện":"Tạm ẩn"}}</td>
